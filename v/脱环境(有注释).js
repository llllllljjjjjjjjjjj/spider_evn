/*getProtoEnvCode函数是生成「内置构造函数」代码的工具
必须传入构造函数：Document、HTMLDocument、Node 等
不能传入实例：document、div、window 等
函数内部大量使用 .name / new proto / .prototype，只有函数才有
*/
// 获取原型环境代码
getProtoEnvCode = function getProtoEnvCode(proto, instanceObj) {
    ...
    try {
        ...
        code += `ldvm.toolsFunc.throwError("${e.name}", "${e.message}")`;
    }
    code += `}\r\n`;
    // 保护原型
    ...
    let proto_protoName = Object.getPrototypeOf(protoObj).constructor.name;
...
getObjEnvcode = function getObjEnvcode(obj, objName, instanceObj) {
    ...
    for(const key in Object.getOwnPropertyDescriptors(obj)) {
        let descriptor = getDescriptor(obj, key, objName, protoName, instanceObj);
